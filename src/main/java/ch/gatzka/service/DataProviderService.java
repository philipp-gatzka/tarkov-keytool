...
public class DataProviderService {
        ...
        for (ItemData data : itemData) {
            ...
            if (itemRepository.existsByTarkovId(data.bsgId)) {
                ...
                if (item.getLastUpdate().isBefore(updated)) {
                    ...
                    itemRepository.update(entity -> entity
                                    .setName(data.name)
                ...
                itemId = itemRepository.insert(entity -> entity
                        .setTarkovId(data.bsgId)
    ...
    private int insertItemPrice(ItemData data, int itemId, GameMode gameMode, LocalDateTime timestamp) {
        if (!itemPriceRepository.existsByItemIdAndGameModeAndTimestamp(itemId, gameMode, timestamp)) {
            return itemPriceRepository.insert(entity -> entity.setItemId(itemId).setMode(gameMode).setTimestamp(timestamp).setFleaPrice(data.avg7daysPrice).setBannedOnFlea(data.bannedOnFlea));
        }
        return 0;
    }
                ...
                for (FetchKeysQuery.Item itemData : data) {
                    if (itemRepository.existsByTarkovId(itemData.id)) {
                        ...
                        if (keyRepository.existsByItemId(item.getId())) {
                            ...
                            keyRepository.insert(entity -> entity.setItemId(item.getId()).setUses(itemData.properties.onItemPropertiesKey.uses));
    ...
    private record ItemData(String uid, String name, Boolean bannedOnFlea, Boolean haveMarketData, String shortName, Integer price, Integer basePrice, Integer avg24hPrice, Integer avg7daysPrice, String traderName, Integer traderPrice,
