...
public class ItemGridView extends VerticalLayout {
    ...
    private void createGrid() {
        ...
        grid.addComponentColumn(entry -> ApplicationUtils.createImageRenderer(entry, ItemGridViewRecord::getIconLink, ItemGridViewRecord::getName)).setHeader(i18n.getTranslation("items.grid.column.image.header", locale));
        ...
        grid.addColumn(gameMode == GameMode.PVP ? "pvpPrice" : "pvePrice").setRenderer(ApplicationUtils.roubleRenderer(entry -> switch (gameMode) {
            case PVE:
                yield entry.getPvePrice();
            case PVP:
                yield entry.getPvpPrice();
        }, locale)).setHeader(i18n.getTranslation("items.grid.column.flea_value.header", locale));

        grid.addColumn("traderPrice").setRenderer(ApplicationUtils.priceRenderer(ItemGridViewRecord::getTraderCurrency, ItemGridViewRecord::getTraderPrice, locale)).setHeader(i18n.getTranslation("items.grid.column.trader_value.header", locale));
